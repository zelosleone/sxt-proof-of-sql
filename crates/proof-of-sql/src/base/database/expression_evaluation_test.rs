...
fn we_can_evaluate_nullable_columns() {
    ...
    assert_eq!(result.values, OwnedColumn::<TestScalar>::Int(vec![10, 20, 30, 40, 50]));
    assert_eq!(result.presence, None);
    
    let expr = col("names");
    let result = table.evaluate_nullable(&expr).unwrap();
    
    assert_eq!(result.values, OwnedColumn::<TestScalar>::VarChar(vec!["Alice".to_string(), "Bob".to_string(), 
...
fn we_can_evaluate_nullable_expressions() {
    ...
    assert_eq!(result.values, OwnedColumn::<TestScalar>::Int(vec![20, 40, 60, 80, 100]));
    assert_eq!(result.presence, None);
    
    let expr = and(col("flags"), col("flags"));
    let result = table.evaluate_nullable(&expr).unwrap();
    
    assert_eq!(result.values, OwnedColumn::<TestScalar>::Boolean(vec![true, false, true, false, true]));
    assert_eq!(result.presence, None);
}
...
fn we_can_handle_null_propagation_in_expressions() {
    ...
    let a = OwnedNullableColumn::<TestScalar>::new(OwnedColumn::<TestScalar>::Int(vec![1, 2, 3, 4, 5]));
    let b = OwnedNullableColumn::<TestScalar>::new(OwnedColumn::<TestScalar>::Int(vec![10, 20, 30, 40, 50]));
    ...
    assert_eq!(result.values, OwnedColumn::<TestScalar>::Int(expected_values));
    ...
    let all_false = OwnedNullableColumn::<TestScalar>::new(OwnedColumn::<TestScalar>::Boolean(vec![false, false, false, false, false]));
    let result = bool_c.element_wise_and(&all_false).unwrap();
    let expected_values = OwnedColumn::<TestScalar>::Boolean(vec![false, false, false, false, false]);
    let expected_presence = Some(vec![
    ...
    // NULL OR true = true
    let all_true = OwnedNullableColumn::<TestScalar>::new(OwnedColumn::<TestScalar>::Boolean(vec![true, true, true, true, true]));
...
fn we_can_convert_nullable_to_non_nullable() {
    ...
    let truly_non_null = OwnedNullableColumn::<TestScalar>::new(
